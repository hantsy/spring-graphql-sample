...
class DemoApplicationTestsWithGraphQLClient {
    ...
    lateinit var client: SSESubscriptionGraphQLClient;

    @BeforeEach
    fun setup() {
        val baseUrl = "http://localhost:$port/subscriptions"
        client = SSESubscriptionGraphQLClient(
            baseUrl,
    ...
    fun testMessages() {
        ...
        val result = client.reactiveExecuteQuery(query, variables).map {
